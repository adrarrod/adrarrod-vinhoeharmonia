...
async function createOrder(deps, payload) {
  ...
  // Baixa o estoque ANTES de gravar o pedido, e de forma atômica: com poucas
  // garrafas por vinho, dois clientes finalizando ao mesmo tempo não podem
  // levar a mesma garrafa. Lança OutOfStockError se faltar saldo.
  const stockDeps = { execute: deps.execute };
  await stock.reserveItems(stockDeps, items);

  let id;
  try {
    ({ id } = await db.insertOrder(deps.execute, row));
  } catch (err) {
    // O pedido não foi gravado: as garrafas voltam para a prateleira.
    await stock.releaseItems(stockDeps, items);
    throw err;
  }
  return { id, ...totals };
}
