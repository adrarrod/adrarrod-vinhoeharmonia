...
function validateOrderPayload(payload) {
  ...
  // pelo menos garantimos sanidade: nada de negativo, NaN ou string. Na retirada
  // não há frete, então aceitamos o campo ausente como zero.
  const freight = d.freightCost;
  const freightOmittedOnPickup = d.type === 'pickup' && (freight === undefined || freight === null);
  if (!freightOmittedOnPickup && !(typeof freight === 'number' && Number.isFinite(freight) && freight >= 0)) {
...
module.exports = { ValidationError, MAX_QTY_PER_ITEM, isAdult, validateOrderPayload, createOrder, listOrdersForAdmin, setOrderStatus };
