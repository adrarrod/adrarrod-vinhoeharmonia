async function createPreference(deps, order) {
  ...
  if (!response.ok) {
    ...
    const causes = Array.isArray(errBody.cause)
      ? errBody.cause.map((c) => `${c.code || ''} ${c.description || ''}`.trim()).join('; ')
      : '';
    const detail = [errBody.message, causes].filter(Boolean).join(' — ');
    throw new Error(`Mercado Pago error ${response.status}: ${detail || 'unknown'}`);
  }
